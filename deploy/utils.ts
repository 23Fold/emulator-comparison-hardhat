...
/**
 * @param {string} data.contract The contract's path and name. E.g., "contracts/erc20/MyERC20Token.sol:MyERC20Token"
 */
...
export const GENERAL_PAYMASTER_PARAMS = utils.getPaymasterParams(
...
export const deployContract = async (
  ...
  const gasUsed = await deployer.estimateDeployGas(artifact, constructorArguments || []);
  log(`Estimated deployment cost: ${ethers.formatEther(deploymentFee)} ETH`);
  log(`Estimated gas used: ${gasUsed}`);
  ...
  if (withPaymaster) {
    ...
    contract = await deployer.deploy(artifact, constructorArguments, undefined, {
      customData: {
        paymasterParams: GENERAL_PAYMASTER_PARAMS,
  ...
  const actualUsedGas = await contract.deploymentTransaction()?.wait();
  log(`Actual gas used: ${actualUsedGas?.gasUsed}`);
